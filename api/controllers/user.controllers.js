...
const { saveSystemRole, loginUserServive, findSystemRoles, findSystemRoleById, findSystemRoleByIdAndUpdate } = require(`../../dependencies/internal-services/user.services`);
...
const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    // calling data service to login user
    const { status, data, error } = await loginUserServive(req.body);
    ...
    if (status === SERVER_ERROR) {
      ...
      // logging error message to the console
      logError(`Requested operation failed! Wrong User Credentials.`);
