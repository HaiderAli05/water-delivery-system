...
const saveSystemPermission = async (systemPermissionData, _creator) => {
  ...
  try {
    ...
    const result = await newSystemPermission.save();
    ...
    logError(`ERROR @ saveSystemPermission -> system-permission.services.js`, error);
    ...
    const [status, err] = [isDuplicateError ? CONFLICT : SERVER_ERROR, isDuplicateError ? `System permission creation failed due to duplicate ${duplicateErrorFields}.` : `System permission creation failed.`];
...
const findSystemPermissions = async (queryScope) => {
  ...
  try {
    ...
    const result = await SystemPermission.find({ isDeleted: false }).populate({ path: `_creator`, select: `fullName systemPermissions` }).select(queryScope).lean().exec();
    ...
    logError(`ERROR @ findSystemPermissions -> system-permission.services.js`, error);
...
const findSystemPermissionById = async (systemPermission, queryScope) => {
  ...
  try {
    ...
    const result = await SystemPermission.findOne({ _id: systemPermission }).populate({ path: `_creator`, select: `fullName allowedSystemPermissions photo` }).select(queryScope).lean().exec();
    ...
    // loggine error messages to the console
    logError(`ERROR @ findSystemPermissionById -> system-permission.services.js`, error);
...
const findSystemPermissionByIdAndUpdate = async (systemPermissionId, updateData, updateBy, queryScope) => {
  ...
  try {
    ...
    // querying database for the requested system permission
    const result = await SystemPermission.findOneAndUpdate({ _id: systemPermissionId, isDeleted: false }, updateData, configParams).select(queryScope).lean().exec();
    ...
    // loggine error messages to the console
    logError(`ERROR @ findSystemPermissionByIdAndUpdate -> system-permission.services.js`, error);
    ...
    // setting value of status and description
    const [status, err] = [isDuplicateError ? CONFLICT : SERVER_ERROR, isDuplicateError ? `System permission update failed due to duplicate ${duplicateErrorFields}.` : `System permission update failed.`];
