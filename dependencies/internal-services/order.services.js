...
const findSystemRoles = async (queryScope) => {
  ...
  try {
    ...
    const result = await Order.find({ isDeleted: false }).populate({ path: `_creator`, select: `fullName systemPermissions` }).select(queryScope).lean().exec();
...
const findSystemRoleById = async (systemRole, queryScope) => {
  ...
  try {
    ...
    const result = await Order.findOne({ _id: systemRole, isDeleted: false }).populate({ path: `_creator`, select: `fullName allowedSystemPermissions photo` }).select(queryScope).lean().exec();
...
const findSystemRoleByIdAndUpdate = async (systemRoleId, updateData, updateBy, queryScope) => {
  ...
  try {
    ...
    const result = await Order.findOneAndUpdate({ _id: systemRoleId, isDeleted: false }, updateQuery, configParams).select(queryScope).lean().exec();
